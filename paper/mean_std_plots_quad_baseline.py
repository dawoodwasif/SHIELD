...
PLOTS = [
    ...
    dict(key='0_aux/avg_rewraw_pos', name='Distance to Target', label='Avg. Distance (meters)', coeff=-1.0/EPISODE_DURATION),
    dict(key='0_aux/avg_num_collisions_after_settle', name='Collision Rate', label='Collisions per Minute', logscale=True, coeff=COLLISIONS_SCALE, clip_min=0.05),
    dict(key='0_aux/avg_rewraw_crash', name='Flight Performance', label='Fraction in Air', coeff=CRASH_GROUND_SCALE, mutate=lambda y: 1 - y, clip_max=1.0),
]
...
def plot(index, interpolated_key, ax):
    ...
    for i in range(len(y_np)):
        if len(y_np[i]) > 10:
            y_np[i] = savgol_filter(y_np[i], min(21, len(y_np[i])//4*2+1), 3)
    ...
    ax.grid(color='#B3B3B3', linestyle='-', linewidth=0.25, alpha=0.3)
    ax.ticklabel_format(style='plain', axis='y', scilimits=(0, 0))

    lw = 2.5

    # Plot with enhanced visuals
    ax.plot(x, y_mean, color=ORANGE, linewidth=lw, antialiased=True, label='QuadSwarm Performance')
    ax.fill_between(x, y_minus_std, y_plus_std, color=ORANGE, alpha=0.3, antialiased=True, linewidth=0.0)
    
    # Add final value annotation
    if len(y_mean) > 0:
        final_val = y_mean[-1]
        if index == 0:  # Reward
            ax.annotate(f'{final_val:.1f}', xy=(x[-1], final_val), 
                       xytext=(x[-1]*1.02, final_val), fontsize=10, 
                       fontweight='bold', color=ORANGE, va='center')
        elif index == 1:  # Distance
            ax.annotate(f'{final_val:.2f}m', xy=(x[-1], final_val), 
                       xytext=(x[-1]*1.02, final_val), fontsize=10, 
                       fontweight='bold', color=ORANGE, va='center')
