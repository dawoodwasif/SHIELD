...
class QuadMultiEncoder(Encoder):
    ...
    def forward(self, obs_dict):
        ...
        if self.use_obstacles:
            ...
            # Safety check: ensure obstacle observations have the expected size
            if obs_obstacles.shape[1] > 0:
                obstacle_embeds = self.obstacle_encoder(obs_obstacles)
                embeddings = torch.cat((embeddings, obstacle_embeds), dim=1)
            else:
                # If no obstacle observations, create zero embeddings
                obstacle_embeds = torch.zeros(batch_size, self.obstacle_encoder[0].out_features, device=obs.device)
                embeddings = torch.cat((embeddings, obstacle_embeds), dim=1)
