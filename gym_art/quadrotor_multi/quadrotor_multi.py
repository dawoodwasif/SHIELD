...
class QuadrotorEnvMulti(gym.Env):
    def __init__(self, num_agents, ep_time, rew_coeff, obs_repr,
                 # Neighbor
                 neighbor_visible_num=6, neighbor_obs_type="pos_vel", collision_hitbox_radius=2.0, collision_falloff_radius=4.0,

                 # Obstacle
                 use_obstacles=False, obst_density=0.1, obst_size=0.5, obst_spawn_area=[10, 10],

                 # Aerodynamics, Numba Speed Up, Scenarios, Room, Replay Buffer, Rendering
                 use_downwash=True, use_numba=False, quads_mode="mix", room_dims=[10, 10, 3], use_replay_buffer=False, quads_view_mode=["chase"],
                 quads_render=False,

                 # Quadrotor Specific (Do Not Change)
                 dynamics_params=None, raw_control=False, raw_control_zero_middle=True,
                 dynamics_randomize_every=None, dynamics_change=None, dyn_sampler_1=None,
                 sense_noise=None, init_random_state=False,
                 # Rendering
                 render_mode='human'
                 ):
        super().__init__()
        ...
        # Set Obs & Act - Fix observation space calculation
        base_obs_space = self.envs[0].observation_space
        
        # Calculate total observation size including obstacles
        total_obs_size = obs_self_size
        
        # Add neighbor observations
        if self.num_use_neighbor_obs > 0:
            neighbor_obs_size = QUADS_NEIGHBOR_OBS_TYPE[neighbor_obs_type]
            total_obs_size += self.num_use_neighbor_obs * neighbor_obs_size

        # Add obstacle observations
        if use_obstacles:
            obstacle_obs_size = 9  # OBST_DIM = 9 for octomap
            total_obs_size += obstacle_obs_size
            
        # Update observation space to match actual size
        if hasattr(base_obs_space, 'shape'):
            from gymnasium.spaces import Box
            self.observation_space = Box(
                low=np.full(total_obs_size, base_obs_space.low[0]),
                high=np.full(total_obs_size, base_obs_space.high[0]),
        ...
        # Add missing attributes for replay buffer compatibility
        self.obst_density = obst_density  # Add this line to fix the AttributeError
        self.obst_size = obst_size        # Add this line for consistency
        ...
        if self.use_obstacles:
            ...
            self.curr_quad_col = []
            # Remove duplicate assignment since we already set it above
            # self.obst_density = obst_density  # This was already here, causing confusion
            self.obst_spawn_area = obst_spawn_area
            self.num_obstacles = int(obst_density * obst_spawn_area[0] * obst_spawn_area[1])
            self.obst_map = None
            # Remove duplicate assignment
            # self.obst_size = obst_size  # This was already here
        ...
        # Neighbor
        self.neighbor_obs = None  # Initialize neighbor_obs to avoid NoneType issues
        self.neighbor_obs_min = -10.0  # Set default minimum value for neighbor observations
        self.neighbor_obs_max = 10.0   # Set default maximum value for neighbor observations

        # Initialize neighborhood observation parameters
        self.neighbor_obs_type = neighbor_obs_type  # Add this line
        self.neighbor_visible_num = neighbor_visible_num  # Add this line
    ...
    def compute_neighbor_obs(self):
        """Compute the neighbor observations for the environment."""
        # Placeholder implementation for neighbor observations
        num_drones = self.num_drones if hasattr(self, 'num_drones') else 0
        obs_dim = self.neighbor_obs_dim if hasattr(self, 'neighbor_obs_dim') else 0
        return np.zeros((num_drones, obs_dim))

    def extend_obs_space(self, obs, closest_drones=None):
        """
        Extends the observation space with information about neighboring drones
        """
        ...
        n_drones = obs.shape[0]
        
        # Set default neighbor_obs_type if not defined
        if not hasattr(self, 'neighbor_obs_type') or self.neighbor_obs_type is None:
            self.neighbor_obs_type = "pos_vel"  # Default to position and velocity
            
        # Set default neighbor_visible_num if not defined
        if not hasattr(self, 'neighbor_visible_num') or self.neighbor_visible_num is None:
            self.neighbor_visible_num = 6  # Default to 6 visible neighbors
        
        # Initialize obs_neighbors with zeros
        if self.neighbor_obs_type == "pos_vel":
            # Position and velocity (6 values per neighbor)
            obs_neighbors = np.zeros((n_drones, self.neighbor_visible_num * 6))
        elif self.neighbor_obs_type == "pos":
            # Only position (3 values per neighbor)
            obs_neighbors = np.zeros((n_drones, self.neighbor_visible_num * 3))
        else:
            # Default to position and velocity if unknown type
            print(f"WARNING: Unknown neighbor_obs_type: {self.neighbor_obs_type}. Defaulting to 'pos_vel'")
            self.neighbor_obs_type = "pos_vel"
            obs_neighbors = np.zeros((n_drones, self.neighbor_visible_num * 6))
            
        # Process each drone's neighbors
        for i in range(n_drones):
            if closest_drones is None:
                # Use all other drones as neighbors (excluding self)
                neighbors = list(range(n_drones))
                neighbors.remove(i)
                # If more drones than visible_num, select closest ones (already sorted by distance)
                if len(neighbors) > self.neighbor_visible_num:
                    neighbors = neighbors[:self.neighbor_visible_num]
            else:
                # Use provided closest drones
                neighbors = closest_drones[i]
            
            # Fill neighbor information
            for j, neigh_idx in enumerate(neighbors):
                if j >= self.neighbor_visible_num:
                    break
                    
                if self.neighbor_obs_type == "pos_vel":
                    # Position (first 3) and velocity (next 3)
                    obs_neighbors[i, j*6:j*6+3] = self.get_drone_position(obs[neigh_idx])
                    obs_neighbors[i, j*6+3:j*6+6] = self.get_drone_velocity(obs[neigh_idx])
                elif self.neighbor_obs_type == "pos":
                    # Only position (3 values)
                    obs_neighbors[i, j*3:j*3+3] = self.get_drone_position(obs[neigh_idx])
        
        # Reshape for final output
        obs_neighbors = np.reshape(obs_neighbors, (obs.shape[0], -1))
    ...
    def reset(self, obst_density=None, obst_size=None):
        ...
        # Ensure neighbor_obs is initialized during reset
        self.neighbor_obs = self.compute_neighbor_obs()
    ...
    def step(self, actions):
        ...
        if any(dones):
            ...
            if not self.saved_in_replay_buffer:
                ...
                # agent_survival_rate: agents that avoid collisions (regardless of goal achievement)
                agent_survival_ratio = 1.0 * np.sum(agent_col_flag_list) / self.num_agents

                # agent_goal_rate: agents that reach goal (regardless of collisions)
                agent_goal_ratio = 1.0 * np.sum(self.reached_goal) / self.num_agents

                # agent_partial_success_rate: reached goal OR survived without major collisions
                agent_partial_success_list = np.logical_or(agent_col_flag_list, self.reached_goal)
                agent_partial_success_ratio = 1.0 * np.sum(agent_partial_success_list) / self.num_agents

                # agent_task_completion_rate: more lenient - considers progress toward goal
                # Based on final distance to goal being small (< 2.0 units) regardless of collisions
                final_distances = np.array([np.mean(self.distance_to_goal[i][-int(self.control_freq):]) if len(self.distance_to_goal[i]) >= self.control_freq 
                                          else np.mean(self.distance_to_goal[i][-5:]) if len(self.distance_to_goal[i]) >= 5 
                                          else float('inf') for i in range(self.num_agents)])
                agent_near_goal_list = final_distances < 2.0  # Within 2 units of goal
                agent_task_completion_ratio = 1.0 * np.sum(agent_near_goal_list) / self.num_agents
                ...
                for i in range(len(infos)):
                    ...
                    # New less strict metrics
                    infos[i]['episode_extra_stats']['metric/agent_survival_rate'] = agent_survival_ratio
                    infos[i]['episode_extra_stats'][f'{scenario_name}/agent_survival_rate'] = agent_survival_ratio
                    
                    infos[i]['episode_extra_stats']['metric/agent_goal_rate'] = agent_goal_ratio
                    infos[i]['episode_extra_stats'][f'{scenario_name}/agent_goal_rate'] = agent_goal_ratio
                    
                    infos[i]['episode_extra_stats']['metric/agent_partial_success_rate'] = agent_partial_success_ratio
                    infos[i]['episode_extra_stats'][f'{scenario_name}/agent_partial_success_rate'] = agent_partial_success_ratio
                    
                    infos[i]['episode_extra_stats']['metric/agent_task_completion_rate'] = agent_task_completion_ratio
                    infos[i]['episode_extra_stats'][f'{scenario_name}/agent_task_completion_rate'] = agent_task_completion_ratio
    ...
    def get_drone_position(self, obs):
        """Extract drone position from observation."""
        # Assuming position is in the first 3 elements of observation
        if isinstance(obs, (list, np.ndarray)) and len(obs) >= 3:
            return obs[:3]
        else:
            return np.zeros(3)

    def get_drone_velocity(self, obs):
        """Extract drone velocity from observation."""
        # Assuming velocity is in elements 3-6 of observation
        if isinstance(obs, (list, np.ndarray)) and len(obs) >= 6:
            return obs[3:6]
        else:
            return np.zeros(3)
