...
class ExperienceReplayWrapper(gym.Wrapper):
    ...
    def new_episode(self):
        ...
        if np.random.uniform(0, 1) < self.replay_buffer_sample_prob and self.replay_buffer and self.env.activate_replay_buffer \
            ...
            # Fix: Add defensive check for obst_density attribute
            if hasattr(replayed_env, 'obst_density'):
                self.curr_obst_density = replayed_env.obst_density
            else:
                # Fallback to default or current value
                self.curr_obst_density = getattr(self.env, 'obst_density', self.curr_obst_density)
                # Also set the attribute on replayed_env to prevent future errors
                replayed_env.obst_density = self.curr_obst_density
