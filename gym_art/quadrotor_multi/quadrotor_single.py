...
# Define observation space size constants
QUADS_OBS_REPR = {
    'xyz_vxyz_R_omega': 18,  # 3 + 3 + 9 + 3
    'xyz_vxyz_R_omega_floor': 19,  # 3 + 3 + 9 + 3 + 1 (floor)
    'xyz_vxyz_quat_omega': 13,  # 3 + 3 + 4 + 3
}
...
class QuadrotorSingle:
    def __init__(self, dynamics_params="DefaultQuad", dynamics_change=None,
        ...
        # Make observation space - ensure it's a proper Gymnasium space
    ...
    def make_observation_space(self):
        """Create a proper Gymnasium observation space."""
        import gymnasium as gym
        
        # Calculate observation dimensions based on obs_repr and neighbor observations
        self_obs_size = QUADS_OBS_REPR[self.obs_repr]
        neighbor_obs_size = QUADS_NEIGHBOR_OBS_TYPE[self.neighbor_obs_type] * self.num_use_neighbor_obs
        total_obs_size = self_obs_size + neighbor_obs_size
        
        # Define bounds for different observation components
        pos_bound = 10.0  # position bounds
        vel_bound = 3.0   # velocity bounds
        rot_bound = 1.0   # rotation matrix bounds
        omega_bound = 40.0  # angular velocity bounds
        floor_bound = 1.0  # floor observation bound
        neighbor_pos_bound = 10.0  # neighbor position bounds
        neighbor_vel_bound = 6.0   # neighbor velocity bounds
        
        # Build low and high arrays
        low = []
        high = []
        
        # Self observations
        if 'xyz' in self.obs_repr:
            low.extend([-pos_bound] * 3)
            high.extend([pos_bound] * 3)
        if 'vxyz' in self.obs_repr:
            low.extend([-vel_bound] * 3)
            high.extend([vel_bound] * 3)
        if 'R' in self.obs_repr:
            low.extend([-rot_bound] * 9)
            high.extend([rot_bound] * 9)
        if 'omega' in self.obs_repr:
            low.extend([-omega_bound] * 3)
            high.extend([omega_bound] * 3)
        if 'floor' in self.obs_repr:
            low.extend([0.0])  # floor distance is always positive
            high.extend([floor_bound])
        
        # Neighbor observations
        for _ in range(self.num_use_neighbor_obs):
            if 'pos' in self.neighbor_obs_type:
                low.extend([-neighbor_pos_bound] * 3)
                high.extend([neighbor_pos_bound] * 3)
            if 'vel' in self.neighbor_obs_type:
                low.extend([-neighbor_vel_bound] * 3)
                high.extend([neighbor_vel_bound] * 3)
        ...
        assert len(low) == total_obs_size, f"Low bounds length {len(low)} != expected {total_obs_size}"
        assert len(high) == total_obs_size, f"High bounds length {len(high)} != expected {total_obs_size}"
        
        return gym.spaces.Box(low=low, high=high, shape=(total_obs_size,), dtype=np.float32)
